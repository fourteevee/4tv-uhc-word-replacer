...
input_word = "" if DEBUG else inputStr("Please enter the word you want to replace: ")
output_word = "" if DEBUG else inputStr("Please enter the word you want to replace it with: ")
                        ...
                        match item:
                            ...
                            case "content":
                                ...
                                for line_unstripped in json_contents[story][object_id][item].split("<br />"):
                                    ...
                                    if "".join(pre_name) in chum_handles.keys():
                                    ...
                                    elif "".join(pre_name) in names.keys():
                                        ...
                                        # the above code.
                                        for length in [7, 9, 13, 14]:
                                            if line[0:length] in names.keys():
                                                key = names["".join(pre_name)]
                                                srch = compile(r"[^" + string.ascii_letters + r"1234567890]+"
                                                               + input_quirked[key].replace("(", "\(").replace(")",
                                                                                                               "\)") + r"[^" +
                                                               string.ascii_letters + r"1234567890]+")
                                                if search(srch, line):
                                                    extras = search(srch, line).group().split(input_quirked[key])
                                                    write_to_mod(line, line.replace(
                                                        extras[0] + input_quirked[key] + extras[1],
                                                        extras[0] + output_quirked[key] + extras[
                                                            1]),
                                                                 last_page_number)
                                    else:
...
def fishpun_converter(content, freq=100):
    ...
    if randint(0, 100) <= freq:
        ...
        for item in FISHPUNS:
            if content == item:
                content = content.replace(item, FISHPUNS[item])
                if caps:
